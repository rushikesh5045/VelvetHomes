...
const ExplorePage = () => {
  ...
  const [globalFilter, setGlobalFilter] = useState({ minPrice: '', maxPrice: '', company: '' });
  const [globalSort, setGlobalSort] = useState({ sortBy: 'price', sortOrder: 'asc' });
  ...
  return (
    <>
      ...
      <div>
        ...
        <Box display="flex" justifyContent="space-evenly" alignItems="start" mb={3}>
          <FilterComponent onFilterChange={handleGlobalFilterChange} uniqueCompanies={uniqueCompanies} companyDetailsMap={companyDetails} />
              ...
              .filter((category) =>
                products.some(
                  (product) =>
                    ...
                    ((globalFilter.minPrice === '' || product.price >= globalFilter.minPrice) &&
                      (globalFilter.maxPrice === '' || product.price <= globalFilter.maxPrice) &&
                      (globalFilter.company === '' || product.company === globalFilter.company))
                )
              )
              .map((category) => (
                <div key={category}>
                      ...
                      .map((product) => (
                        <Grid item key={product._id}>
                          <Link to={`/products/${product._id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                            <ProductCard product={{ ...product, company: companyDetails[product.company] }} />
                          </Link>
                        </Grid>
                      ))}
                  </Grid>
                  {visibleProducts[category] <
                    products.filter((product) => product.cat === category).filter((product) => {
                        const { minPrice, maxPrice, company } = globalFilter;
                        return (
                          (minPrice === '' || product.price >= minPrice) &&
                          (maxPrice === '' || product.price <= maxPrice) &&
