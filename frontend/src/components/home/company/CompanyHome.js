...
const CompanyHome = () => {
  ...
  useEffect(() => {
    const fetchRecentPurchases = async () => {
      ...
      try {
        ...
        const filteredProductDetails = productDetailsArray.filter((productDetail, index, array) =>
          productDetail && productDetail.company === companyId && array.findIndex(item => item && item._id === productDetail._id) === index
        );
  ...
  const sortedProductsWithSales = productsWithSales.sort((a, b) => b.sales - a.sales);
  const mostSold = sortedProductsWithSales[0];
  const mostSoldProductDetails = productDetails.find(product => product._id === mostSold?.productId);
    ...
    <>
      ...
      <div className="home">
        ...
        <div className="most-sold-details">
          {mostSoldProductDetails && (
            <>
              <p style={{backgroundColor:"black",color:"white",fontSize:"larger",padding:"1rem"}}>Last purchased: {lastPurchaseDate ? new Date(lastPurchaseDate).toLocaleString() : 'N/A'}</p>
              <p style={{backgroundColor:"black",color:"white",fontSize:"larger",padding:"1rem"}}>Total items sold: {mostSold ? mostSold.sales : 0}</p>
            </>
          )}
        </div>
      </div>
...
<div style={{display:"flex",gap:"20px"}}>
    <div><Card variant="solid"  invertedColors style={{width:"200px",backgroundColor:"black"}}>
      ...
      <CardActions>
        <Button variant="solid" size="sm" onClick={seeBreakDownHandler}>
          See breakdown
        </Button>
      </CardActions>
    </Card>
    ...
    <tbody>
      {productsWithSales.map((product) => (
        ...
        <h1 style={{ fontFamily: "monospace", color: "rgb(110 27 27)" }}>
          ...
          LifeTimeSales(a graph with filter of finanicial year) a axis months y axis amount.
          </h1>
        </div>
    </>
  );
